...
def analyze_user_response(text: str) -> tuple[Dict[str, float], Dict[str, float]]:
    """分析使用者回應，回傳情緒和壓力分數"""
    ...
    # 執行分析
    sentiment_raw = SENTIMENT_MODEL.analyze(text)
    stress_raw = STRESS_MODEL.analyze(text)
...
def start_test():
    ...
    # 生成第一個問題（沒有上下文）
    question = generate_question(0, previous_answers=None)
...
def submit_answer(req: AnswerRequest):
    ...
    # 分析使用者回應
    sentiment_scores, stress_scores = analyze_user_response(req.answer_text)
